...
import AddMedia from "@/pages/admin/components/addMedia.vue";
import DeleteMedia from "@/pages/admin/components/deleteMedia.vue";
...
const getListMedia = async () => {
  try {
    const res = await useAdminService().listMedia(
...
<template>
  <ClientOnly>
    <NuxtLayout>
      <div class="relative">
        <h2 class="fs-36 fw-600 text-black-700 mb-12">Danh sách tài liệu</h2>
        <div class="flex items-center justify-between mb-12">
          <button class="find" @click="open = true">Thêm</button>
          <div class="flex items-center gap-2">
            <input v-model="search" type="text" placeholder="Tìm kiếm" />
            <button class="find" @click="getListMedia">Tìm kiếm</button>
          </div>
        </div>
        <table class="mb-12">
          <tr>
          ...
          <tr v-for="(i, index) in list" :key="index">
            ...
            <td>{{ i.type }}</td>
            <td>{{ i.ref_id }}</td>
            <td><a :href="`${i.src}`" class="text-blue">Tải xuống</a></td>
            <td>
            ...
            <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
              ...
              >
                <DialogPanel class="relative transform overflow-hidden rounded-lg bg-white p-8 text-left shadow-xl transition-all mw-640 w-1/2">
                  <delete-media @close="openDelete = false" @delete-success="deleteSuccess" :id="id" />
                </DialogPanel>
              </TransitionChild>
            </div>
          </div>
        ...
        <Dialog as="div" class="relative z-10" @close="open = false">
          ...
          <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
            <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
              ...
              >
                <DialogPanel class="relative transform overflow-hidden rounded-lg bg-white p-8 text-left shadow-xl transition-all mw-640 w-1/2">
                  <add-media @close="open = false" @add-success="addSuccess" />
                </DialogPanel>
              </TransitionChild>
            </div>
          </div>
